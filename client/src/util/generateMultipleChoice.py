    ...
    for phrase in phrases:
        ...
        if len(words) > 3:
            ...
            options = [missing_word] + random.sample(
                [w for w in all_words if w and w != missing_word], min(num_options - 1, len(all_words) - 1))
...
def generate_translation_questions(file_name, num_questions=10, num_options=4):
    ...
    for word in selected_words:
        ...
        # Generate multiple-choice options
        distractors = [t for t in translations.values() if t and t !=
                       correct_translation]
        options = [correct_translation] + \
            random.sample(distractors, min(num_options - 1, len(distractors)))
        random.shuffle(options)
